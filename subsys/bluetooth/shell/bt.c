		     ...
		     const uint8_t status)
{
	if (status == BT_HCI_ERR_SUCCESS) {
		shell_print(ctx_shell, "Subrate parameters changed successfully.");
		shell_print(ctx_shell, "Type `bt info` to view updated parameters.");
	} else {
		shell_print(ctx_shell, "Subrate parameter change failed (status 0x%02x)", status);
	}
}
#endif
